...
public partial class PlayerInputsSystem : SystemBase
{
	...
	protected override void OnUpdate()
	{
		...
		foreach (var (playerInputs, player) in SystemAPI.Query<RefRW<PlayerInputs>, Player>())
		{
			...
			playerInputs.ValueRW.Look = _defaultActionsMap.LookPosition.ReadValue<Vector2>(); //_defaultActionsMap.LookDelta.ReadValue<Vector2>();
			...
			if (_defaultActionsMap.Choose1.WasPressedThisFrame())
			{
				playerInputs.ValueRW.ChooseSlot = (byte)1;
			}
			if (_defaultActionsMap.Choose2.WasPressedThisFrame())
			{
				playerInputs.ValueRW.ChooseSlot = (byte)2;
			}
			if (_defaultActionsMap.Choose3.WasPressedThisFrame())
			{
				playerInputs.ValueRW.ChooseSlot = (byte)3;
			}
			if (_defaultActionsMap.Choose4.WasPressedThisFrame())
			{
				playerInputs.ValueRW.ChooseSlot = (byte)4;
			}
			if (_defaultActionsMap.Choose5.WasPressedThisFrame())
			{
				playerInputs.ValueRW.ChooseSlot = (byte)5;
			}
	...
	public void OnUpdate(ref SystemState state)
	{
					 ...
					 .WithAll<Simulate>())
		{
			if (SystemAPI.HasComponent<CharacterControl>(player.ControlledCharacter) && SystemAPI.HasComponent<CharacterStateMachine>(player.ControlledCharacter))
			{
				...
				characterControl.LookVector = playerInputs.ValueRO.Look;
				...
				characterControl.ChooseSlot = playerInputs.ValueRO.ChooseSlot;
